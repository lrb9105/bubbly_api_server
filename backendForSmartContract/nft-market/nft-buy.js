    ...
    maria.query(queryStr, datas, async function(err, rows, fields){
            if(!err){
                ...
                for(let i = 0; i < accountObj.assets.length; i ++ ){
                    if(accountObj.assets[i]["asset-id"] == 9){
                        console.log("amount: " + accountObj.assets[i]["amount"]);
                        console.log("price: " + Number(buyPrice));
                        
                        // 보유한 버블양이 nft가격보다 적다면
                        if(accountObj.assets[i]["amount"] < Number(buyPrice)){
                            return resolve("보유한 토큰양이 부족해 nft를 구매할 수 없습니다.");
                        } else {
                            break;
                        }
                    }
                        ...
                        await maria.query(queryStr, datas, async function(err, rows, fields){
                            if(!err){
                                ...
                                await maria.query(queryStr, datas, function(err, rows, fields){
                                    if(!err){
                                        console.log("성공");
                                        return resolve("NFT 구매가 완료되었습니다.");
                                    } else {
                                        console.log("실패1");
                                        console.log(err);
                                        return resolve("NFT 구매에 실패하였습니다.");
                                    }
                                });
                            } else {
                                console.log("실패2");
